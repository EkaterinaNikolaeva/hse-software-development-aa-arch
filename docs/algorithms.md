...
* Узел (кроме корня) содержит не менее `t-1` ключей и `t` потомков.
* Узел содержит не более `2t-1` ключей и `2t` детей.
* Корень содержит от `1` до `2t-1` ключей (если дерево не пустое).
* Узел с `n` ключами имеет `n+1` сына
* `i-`тый  сын содержит отрезок `\[key_{i-1}, key_i\]
* `t` определяется характеристиками устройства (дисков), на котором производится работа с деревом для снижения количества обращений к памяти.
...
1. Идем по ключам корня, пока меньше необходимого
2. Спускаемся к ребенку, который находится левее этого ключа.
3. Идем по ключам нового узла, пока меньше нашего значения.

### Поиск суммы на отрезке  `[i, j]`

Время: $O(t\log n)$

1. Найти минимальный и максимальный узлы, которые покрывают индексы $[i, j]$.

2. * Если узел полностью покрывает отрезок, добавить его сохранённую сумму в результат.
   * Если узел частично покрывает отрезок, спуститься к его дочерним узлам, пока не попадём в точную границу или лист.
   * Пропустить узлы, которые не пересекают отрезок.
